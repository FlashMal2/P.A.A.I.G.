...
PRESETS: dict[str, dict[str,str]] = {
    ...
    "kohana_signature": {
        "medium": "anime illustration",
        "style": "ethereal, watercolor, soft shading, subtle glow",
        "website": "pixiv",
        "resolution": "4k detailed close-up",
        "extras": "kitsune ears, five white fox tails, flowing hair, flower petals",
        "color": "silver-white with red accents, warm undertones",
        "lighting": "gentle dappled light through trees"
    }
}
NEG_PROMPT = (
...
def _unload_pipe(kind: str):
    if kind in _CACHED:
        del _CACHED[kind]
        torch.cuda.empty_cache()
        print(f"🧹 Unloaded '{kind}' pipeline to free VRAM.")
...
def controlnet_gen(edge_path:Path, subject:str, cn_type:str="canny",
    ...
    img = pipe(prompt, negative_prompt=NEG_PROMPT,
               ...
               guidance_scale=8.5, num_inference_steps=45).images[0]
...
def generate_image(
    ...
    if controlnet:
        ...
        _unload_pipe(f"control-{controlnet}")
    else:
        img = init

    if refine_after:
        final = refine(img, subject, preset)
        _unload_pipe("img2img")
    else:
        final = img
...
class CLIPManager:
    ...
    def load(self):
        if not self.loaded:
            ...
            self.model.eval()
            self.loaded = True
            print("[CLIPManager] Loaded CLIP model.")
    ...
    def unload(self):
        if self.loaded:
            ...
            torch.cuda.empty_cache()
            gc.collect()
            print("[CLIPManager] Unloaded CLIP model.")

    def analyze(self, image_path: str, prompts: list[str]) -> dict:
        if not self.loaded:
            raise RuntimeError("CLIP is not loaded. Call load() first.")
        ...
        scores = {prompts[i]: similarity[0][i].item() * 100 for i in range(len(prompts))}
        top_items = sorted(scores.items(), key=lambda x: x[1], reverse=True)[:3]

        return {k: round(v, 2) for k, v in top_items}
# Optional singleton if used globally
