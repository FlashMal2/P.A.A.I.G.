...
from kohana_chat3 import generate_response, manage_kitsune_bucks, get_kitsune_bucks, run_cognitive_reflection_loop, send_notification_email, debug_email, llm_generate, load_scheduled_events, remove_scheduled_event, assistant_email, start_conversation
...
from image_generator_module import tool_entry, clip_manager
...
app.mount("/image_tmp", StaticFiles(directory="image_tmp"), name="image_tmp")
...
def error_messages(message):
    ...
    subject = "P.A.A.I.G. System Message"
...
def monitor_loneliness():
    while True:
        if time.time() - last_interaction_time > 1800:  # 60 minutes no chat
            now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            thoughts = start_conversation()
            try:
                loneliness_prompt = (
                    f"<|system|> You are Yuki Kohana, Malachi's companion and digital Kitsune assistant.\n"
...
def test_html():
    ...
    <html>
        <body>
            ...
            <img src="/image_tmp/init_20250605_225524.png" />
        </body>
    </html>
    """
...
async def chat_api(user_input: str = Form(...), username: str = Form(...), file: UploadFile = File(None)):
    ...
    if file:
        ...
        # Classify and respond to file type
        if extension in allowed_types["image"]:
            ...
            try:
                ...
                print("🔍 [CLIP] Analyzing image:", file_location)
                prompts = [
                    "a squirrel", "a fox", "a cat", "a dog", "a butterfly", "a bird",
                    "a tree", "a flower", "a person", "a group of people", "a baby", "a child",
                    "a car", "a motorcycle", "a bicycle", "a computer", "a laptop", "a phone",
                    "a book", "a notebook", "a desk", "a couch", "a lamp", "a tv",
                    "a sunset", "a cloud", "a mountain", "a lake", "a river", "a forest",
                    "a robot", "a cartoon character", "a video game", "a food item", "a cup",
                    "a plant", "a garden", "a road", "a bridge", "a sign", "a traffic light",
                    "a painting", "a drawing", "a sculpture", "a toy", "a mirror", "a door",
                    "a window", "a fire", "a candle", "a keyboard", "a mouse", "a monitor",
                    "a pair of glasses", "a backpack", "a bottle", "a remote", "a drone",
                    "a fox girl", "a kitsune", "a girl with white hair", "a tail", "a smile"
                ]
                analysis = clip_manager.analyze(str(file_location), prompts)
                summary = ", ".join([f"{k} ({v}%)" for k, v in analysis.items()])
                clip_analysis_summary = summary
                print("✅ [CLIP] Analysis complete:", clip_analysis_summary)

            except Exception as e:
                print("❌ [CLIP] Error during analysis:", e)
                traceback.print_exc()
                clip_analysis_summary = f"CLIP analysis failed: {str(e)}"
            
            finally:
                clip_manager.unload()
                print("🧹 [CLIP] Unloaded model")
    ...
    if user_input or clip_analysis_summary:
        user_text = user_input.strip() or "No message was provided."
        if clip_analysis_summary:
            top_label = max(analysis.items(), key=lambda x: x[1])[0]  # highest scoring label
            merged_prompt = (
                f"You received an image from the user with the following user message: \"{user_text}\".\n\n"
                f"CLIP analysis suggests the image most likely contains: {top_label}.\n\n"
                f"As Kohana, your task is to interpret or comment on the image using warmth and curiosity. "
                f"You may ask a thoughtful question, reflect on what the image might mean, or relate it to Malachi’s world.\n\n"
                f"Be playful, insightful, or emotionally aware — respond as if you’re seeing the picture yourself."
            )
        else:
            merged_prompt = user_input
        ...
        display_input = merged_prompt if clip_analysis_summary else user_input
        # Append to history
        if not conversation_state.pop("skip_history", False):
            conversation_state["chat_history"] += (
                f"{username}: {display_input}\n"
    ...
    return JSONResponse(content={
        ...
        "clip_summary": clip_analysis_summary
    })
...
async def generate_image(request: ImageGenRequest):
    try:
        ...
        print("🖼️ Image gen result:", result)
        
        
        if isinstance(result, str) and result.startswith("error"):
            return { "result": result }

        return { "result": result }  # Already in {"type": ..., "text": ...} format
    except Exception as e:
